...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options["wipe"]:
            ...
            # CI cronjob that reseeds daily with --wipe --force.
        ...
        # ── #1125: fixtures for resource families schemathesis_hooks.py (#1120)
        # otherwise has no seeded row to pull a real id from. Deliberately run
        # *after* card/movement/archival generation and use fixed literals
        # rather than `random.*` — inserting these earlier, or drawing from the
        # shared `random` stream, would shift every subsequent random draw and
        # silently change the committed sample-boards/demo_board.json/.csv
        # snapshot's card corpus for reasons unrelated to this fixture set.
