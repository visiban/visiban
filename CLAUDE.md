...
- **Parallel/multi-issue work uses `scripts/wt`** — when juggling multiple issues or running parallel agent sessions, create an isolated worktree with `scripts/wt new <issue>` (branches off latest `origin/main` into a sibling `../visiban-wt/` dir, symlinks `backend/.venv` + `frontend/node_modules`, and shares the running Docker stack via `COMPOSE_PROJECT_NAME=visiban` in a generated `.envrc`). Backend tests are isolated per worktree via a seeded `backend/.env` pointing at its own SQLite database, and frontend e2e via a `WT_E2E_PORT` `.envrc` export that `playwright.config.ts` reads — so parallel worktrees never collide on the same test data or dev-server port. `new`/`claim` also apply a `status::wip` GitLab label so parallel agents don't grab the same issue (`remove`/`prune`/`release` clear it), and `scripts/wt stash` gives each worktree its own stash slot (plain `git stash` is repo-wide). WIP cap is 10; `scripts/wt prune` removes worktrees whose branches merged and were deleted on origin, `scripts/wt remove <issue>` removes one. This prevents one session's `git checkout` from swapping the working tree under another. These human-managed, issue-named trees are distinct from Claude Code's own session-scoped `.claude/worktrees/agent-*` trees — prefer `wt` for deliberate multi-issue work. Run `scripts/wt help` for all subcommands.
