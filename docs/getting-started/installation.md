...
| `EMAIL_VERIFICATION` | No | Email verification mode: `optional` (default — sends verification email but allows login without it), `none` (no verification required, works without SMTP), or `mandatory` (blocks login until email is verified). `ACCOUNT_EMAIL_VERIFICATION` is a deprecated alias for this setting and will be removed in a future release. |
