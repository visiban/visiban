...
- Extracted shared `ModalWrapper` component (`frontend/src/components/shared/ModalWrapper.tsx`) — all 11 modals now use consistent backdrop (`bg-black/60`), panel styling (`bg-slate-800 border border-slate-700 rounded-lg shadow-xl`), close button (SVG X icon with `aria-label`), and ARIA attributes (`role="dialog"`, `aria-modal="true"`); eliminates hand-rolled variations across AddColumnModal, EditColumnModal, CreateBoardModal, ImportBoardModal, MoveBoardModal, BoardMembersModal, BoardSettingsModal, EditSwimlaneModal, AddSwimlaneModal, ProfileModal, ForceChangePasswordModal, and KeyboardShortcutsOverlay (#427)
...
- Card title text size corrected from `text-xs font-medium` to `text-sm`, column header name weight corrected from `font-semibold` to `font-medium`, and swimlane name corrected from `font-bold text-white` to `text-slate-300` — all now match the design system spec in `frontend/CLAUDE.md` (#431)
