...
import type { Board, BoardFull, BoardMembership, BoardTemplate, BoardPublic, BoardMovement, Column, Swimlane, Label } from "../types";
...
export const getBoardMovements = (boardId: number, params: Record<string, string> = {}) =>
  client.get<{ results: BoardMovement[]; count: number; offset: number; page_size: number }>(`/api/boards/${boardId}/movements/`, { params }).then((r) => r.data);
