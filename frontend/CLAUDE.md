...
| `compact` | (admin opt-in) | Adds a second label (2 + `+N`), due date when not folded into the urgency badge, weight pill (`>1`), attachment count. Still suppresses the priority badge (the colored card border carries priority) and last-moved text. |
...
- The badge tone follows the design tokens (`text-danger` / `text-warning` / `text-fg-secondary` / `text-info`) — no filled background, no second-tier border.
- When the urgency badge is `overdue` or `due-soon`, the card face suppresses the standalone due-date pill so the date is not duplicated.
