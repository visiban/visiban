...
- **Lens view-comfort state splits by ownership: collapse/focus → URL, density → user pref.** Swimlane collapse (`?lens_collapsed=`, comma-joined `encodeURIComponent`'d keys) and single-lane focus (`?lens_focus=<key>`) live in the URL because a lens link is a shareable snapshot — the arrangement travels with the link (consistent with the `?column_dim=`/`?swimlane_dim=` pivot params). Both are validated against the live `data.swimlanes` key set on read and silently dropped when stale (after a re-pivot or truncation). Compact card **density** is a personal reading habit → `useLensDensityPref` (`user:prefs:lens-density`), never the URL. The collapse chevron + focus crosshair are **forked read-only affordances** on `LensSwimlaneRow` (copy the native `SwimlaneRow` SVGs; chevron always visible, crosshair hover-reveal with `focus:opacity-100`); focus renders a `LensFocusBanner` (`bg-primary/15`, **not** `bg-info/15`) stacked *below* the provenance banner and outside the scroll container, with Escape-to-exit at `useEscapeStack` priority 12. Compact density drops the label/milestone/branch-MR/avatar metadata (keeping `#number` + state pill + multi-lane indicator + single-line title) rather than shrinking it — removing the icon row, not crowding it.
